...
function createDataProxyRouter({ twelveDataKey, tradierToken, lastPriceOf, futuresHub }) {
  ...
  // Allowed timeframes - both Twelve Data and Massive natively support all
  // of these directly (confirmed against Twelve Data's docs), so no
  // custom aggregation is needed on our end for any of them.
  const ALLOWED_INTERVALS = new Set(['1min', '5min', '15min', '30min', '1h', '1day']);
  ...
  router.get('/bars', async (req, res) => {
    ...
    try {
      const bars = await cached(`bars:${symbol}:${interval}`, 20_000, async () => {
        ...
        const url = `https://api.twelvedata.com/time_series` +
          `?symbol=${encodeURIComponent(symbol)}&interval=${interval}&outputsize=260` +
          `&timezone=UTC&prepost=true&apikey=${twelveDataKey}`;
