...
function createDataProxyRouter({ twelveDataKey, tradierToken, lastPriceOf }) {
  const router = express.Router();

  // ---------------------------------------------------------------
  // GET /api/bars?symbol=AAPL
  // Historical 1-min bars for seeding the candlestick chart.
  // Cached 20s - plenty of browsers can load the same symbol without
  // each one burning a separate Twelve Data request.
  // ---------------------------------------------------------------
  router.get('/bars', async (req, res) => {
    ...
    try {
      const bars = await cached(`bars:${symbol}`, 20_000, async () => {
        ...
        if (values.length === 0) throw new Error('Twelve Data returned no bars for this symbol');
  ...
  router.get('/expirations', async (req, res) => {
    ...
    try {
      const dates = await cached(`expirations:${symbol}`, 5 * 60_000, async () => {
        ...
        if (!res2.ok) throw new Error(`Tradier expirations error: ${data.fault?.faultstring || res2.status}`);
  ...
  router.get('/chain', async (req, res) => {
    ...
    try {
      const options = await cached(`chain:${symbol}:${expiration}`, 15_000, async () => {
        ...
        const data = await res2.json();
        if (!res2.ok) throw new Error(`Tradier chain error: ${data.fault?.faultstring || res2.status}`);
