...
function createFuturesHub({ apiKey, onPrice, onBigTrade }) {
  ...
  async function resolveFrontMonth(root) {
    return getCached(`frontmonth:${root}`, 24 * 60 * 60_000, async () => {
      ...
      // fetch contracts and pick the earliest still-unexpired
      // last_trade_date ourselves instead of relying on the API to filter
      // or order them for us.
      //
      // Deliberately NOT passing active=true - confirmed unreliable twice
      // now: it previously returned NQH0 (a March 2020 contract, already
      // long expired) when the date param was omitted, and separately -
      // the reason this comment exists - it excluded GCZ6 entirely from
      // GC's results despite GCZ6 genuinely trading live at the time
      // (real block prints observed directly). Whatever Massive's
      // "active" flag actually tracks internally, it doesn't reliably
      // mean "still trading" for every root - GC's monthly (not
      // quarterly) contract cycle may follow different internal rules
      // than NQ/ES. Since we already filter down to the earliest
      // unexpired last_trade_date ourselves below, we don't need Massive's
      // active flag to do that filtering for us at all.
      const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
      const url = `https://api.massive.com/futures/v1/contracts` +
        `?product_code=${encodeURIComponent(root)}&date=${today}&limit=50&apiKey=${apiKey}`;
      ...
      if (results.length === 0) {
        console.error(`[futures] Massive front-month lookup returned no contracts for ${root}:`, JSON.stringify(data));
        throw new Error(`No contract found for ${root}`);
      }

      // Our own definition of "front month": the earliest last_trade_date
      // that hasn't passed yet. This is the piece of logic actually
      // responsible for correctness now that we're not leaning on
      // Massive's active flag - a contract with no last_trade_date, or
      // one that's already expired (< today), is excluded here rather
      // than trusting the API to have already excluded it for us.
      const nearest = results.reduce((soonest, c) => {
        if (!c.last_trade_date || c.last_trade_date < today) return soonest;
  ...
  async function fetchBars(ticker, interval) {
    ...
    // 1-minute bars only cover ~4.3 hours at the normal 260-bar count -
    // far short of the full day flow_events can span (the ES/NQ/GC
    // background watch accumulates continuously). Without more 1-minute
    // history to anchor them to, older flow events have no correctly-
    // positioned candle to attach a chart marker to at all. 1440 = a
    // full 24 hours, matching how far back saved flow can realistically
    // go. Other intervals (5min, 15min, etc.) already span a wide enough
    // window at 260 bars, so they're left as-is - this is deliberately
    // targeted at the one specific gap, not a blanket increase.
    const limit = resolution === '1min' ? 1440 : 260;
    return getCached(`bars:${ticker}:${resolution}`, 20_000, async () => {
      const url = `https://api.massive.com/futures/v1/aggs/${encodeURIComponent(ticker)}` +
        `?resolution=${resolution}&limit=${limit}&sort=window_start.desc&apiKey=${apiKey}`;
