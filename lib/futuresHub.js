...
function createFuturesHub({ apiKey, onPrice, onBigTrade }) {
  ...
  function handleTrade(msg) {
    ...
    // TEMP DIAGNOSTIC: logs the RAW feed message for every trade big enough
    // to show up in the Live Flow sidebar/chart, so an unusually large
    // print (e.g. a huge contract count) can be checked against exactly
    // what Massive actually sent, not just our computed dollarValue.
    // Remove once large-print verification is no longer needed.
    console.log(
