...
function createFuturesHub({ apiKey, onPrice, onBigTrade }) {
  ...
  function handleTrade(msg) {
    ...
    for (const target of targets) {
      ...
      onBigTrade?.(target, { price, size, dollarValue, side, timeMs: msg.t, sourceSymbol: tradedSymbol !== target ? tradedSymbol : undefined });
    }
  }
    ...
    socket.on('message', (raw) => {
      ...
      for (const msg of messages) {
        if (msg.ev === 'status' && msg.status === 'auth_success') {
          ...
          onPrice(msg.sym, { price: msg.c, timeMs: msg.s });
        } else if (msg.ev === 'Q') {
