  ...
  async function resolveFrontMonth(root) {
    return getCached(`frontmonth:${root}`, 24 * 60 * 60_000, async () => {
      const url = `https://api.massive.com/futures/v1/contracts` +
        `?product_code=${encodeURIComponent(root)}&active=true&sort=last_trade_date.asc&limit=1&apiKey=${apiKey}`;
      ...
      if (!ticker) {
        console.error(`[futures] Massive front-month lookup returned no active contract for ${root}:`, JSON.stringify(data));
        throw new Error(`No active contract found for ${root}`);
      }
      console.log(`[futures] Resolved ${root} -> ${ticker}`);
      return ticker;
    });
  }
    ...
    socket.on('message', (raw) => {
      ...
      for (const msg of messages) {
        if (msg.ev === 'status' && msg.status === 'auth_success') {
          ...
          onPrice(msg.sym, { price: msg.c, open: msg.o, high: msg.h, low: msg.l, close: msg.c, timeMs: msg.s });
        } else if (msg.ev === 'Q') {
