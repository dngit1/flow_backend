...
//
// IMPORTANT LIMITATION: on the free "Basic" tier, Massive has NO WebSocket
// access at all - only REST. So "live" price here means polling the latest
// 1-minute bar on an interval, not true tick-by-tick streaming like Alpaca
// gives us for stocks. Expect updates every ~25s, not instantly.
//
...
function createFuturesHub({ apiKey, onPrice }) {
  ...
  async function fetchBars(ticker) {
    return getCached(`bars:${ticker}`, 20_000, async () => {
      const end = new Date();
      const start = new Date(end.getTime() - 24 * 60 * 60 * 1000); // wide window, futures trade nearly 24/5
      const fmt = (d) => d.toISOString().slice(0, 10);

      const url = `https://api.massive.com/v2/aggs/ticker/${encodeURIComponent(ticker)}` +
        `/range/1/minute/${fmt(start)}/${fmt(end)}?apiKey=${apiKey}&sort=asc&limit=1000`;

      const res = await fetch(url);
      const data = await res.json();
      if (!res.ok) throw new Error(`Massive request failed: ${data.error || res.status}`);

      const results = data.results || [];
      if (results.length === 0) throw new Error(`No bars returned for ${ticker}`);

      // Polygon/Massive aggs shape: { t: unix ms, o, h, l, c, v }
      return results.slice(-60).map((bar) => ({
