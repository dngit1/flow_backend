...
function createFuturesHub({ apiKey, onPrice, onBigTrade }) {
  ...
  const INTERVAL_TO_RESOLUTION = {
    ...
    '30min': '30min',
    '1h': '60min',
    // Massive's documented resolution units are sec/min/hour/session/week/
    // month/quarter/year - notably no "day" unit exists. "session" is the
    // best guess for "one trading day" worth of bars, but this one is
    // genuinely less certain than the others - if 1-day futures charts
    // come back empty/error, check Render logs for the exact Massive
    // error message first.
    '1day': 'session',
  };
  ...
  function handleTrade(msg) {
    ...
    if (watchers.has(tradedSymbol)) {
      onPrice(tradedSymbol, { price, timeMs: msg.t });
    }
    ...
    // TEMP DIAGNOSTIC: logs EVERY tick on a ticker that's currently someone's
    // flow-only sibling sub (not directly watched by anyone), regardless of
    // whether it clears the $3M bar. Answers two separate questions at once:
    //   1. Is the sibling's T channel even delivering trades? (if this never
    //      logs, it's a subscription/delivery bug, not a threshold issue)
    //   2. What dollar values are typical for this contract vs. the $3M
    //      threshold? (if it logs often but "qualifies" is always false,
    //      it's just a threshold/rarity issue, not a bug)
    // Remove once the sibling-flow investigation is resolved.
    if (flowOnlyRequestedBy.has(tradedSymbol) && !watchers.has(tradedSymbol)) {
      console.log(
        `[futures][sibling-debug] ${tradedSymbol}: price=${price} size=${size} ` +
        `multiplier=${multiplier} dollarValue=$${Math.round(dollarValue).toLocaleString()} ` +
        `threshold=$${FUTURES_BLOCK_TRADE_THRESHOLD.toLocaleString()} ` +
        `qualifies=${dollarValue >= FUTURES_BLOCK_TRADE_THRESHOLD} ` +
        `requestedBy=${[...flowOnlyRequestedBy.get(tradedSymbol)].join(',')}`
      );
    }
    ...
    socket.on('message', (raw) => {
      ...
      for (const msg of messages) {
        if (msg.ev === 'status' && msg.status === 'auth_success') {
          ...
          onPrice(msg.sym, { price: Number(msg.c), open: Number(msg.o), high: Number(msg.h), low: Number(msg.l), close: Number(msg.c), timeMs: msg.s });
        } else if (msg.ev === 'Q') {
