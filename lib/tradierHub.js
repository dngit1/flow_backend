    ...
    async initLeaderboard() {
      ...
      for (const symbol of LEADERBOARD_SYMBOLS) {
        try {
          ...
          // No spot price handy here (this runs at startup, independent of
          // any client's Alpaca subscription) - use the chain's median
          // strike as a rough center point for the near-the-money filter.
          const strikes = options.map((o) => o.strike).sort((a, b) => a - b);
          const roughSpot = strikes[Math.floor(strikes.length / 2)] || 0;

          const nearMoney = filterNearTheMoney(options, roughSpot, NEAR_MONEY_STRIKES);
          nearMoney.forEach((o) => {
