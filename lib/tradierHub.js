...
function createTradierHub({ token, onFlow }) {
  ...
  return {
    ...
    async watch(clientId, symbol, expiration, spotPrice) {
      ...
      const usedFallback = !spotPrice;
      const centerUsed = spotPrice || options[0]?.strike || 0;
      ...
      console.log(`[tradier] WATCH ${symbol} ${expiration} client=${clientId.slice(0, 8)} spotPrice=${spotPrice || 'NONE'}${usedFallback ? ' (FALLBACK USED!)' : ''} center=${centerUsed} strikes=[${strikeList.join(',')}]`);
