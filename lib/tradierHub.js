...
// Mirrors the frontend's MIN_FLOW_PREMIUM (index.html) - the threshold
// below which a trade never shows up as "flow" at all, just routine
// small activity on a watched contract. Confirmed as a real, significant
// gap: history saving had NO threshold check anywhere, saving every
// single matched trade regardless of size as long as it touched any
// watched contract - not just the duplicate-recording bug fixed
// separately (recentlyRecordedTrades below), likely the larger
// contributor to SPY's confirmed 102,000+ row count. Kept as its own
// constant rather than importing the frontend's, since these are
// separate codebases - if MIN_FLOW_PREMIUM ever changes on the frontend,
// this needs updating to match.
const MIN_FLOW_PREMIUM_FOR_HISTORY = 25_000;
...
const MAX_TOTAL_CONTRACTS = 1800; // reduced from 4000 - that number was based purely on OUR OWN observed contract counts, without knowing Tradier enforces its own real (deliberately unpublished - see their docs: "we do not publish the symbol limits... we monitor for abuse") hard ceiling. Confirmed directly in production: 2406 symbols got the connection rejected outright with code=1007 "too many symbols requested", repeating every ~1s since our own reconnect logic just retried with the same too-large list each time. 1800 leaves real margin below that confirmed failure point - not a guess at Tradier's exact undocumented number, just deliberately conservative. If leaderboard + background watch + normal client activity genuinely need more room than this comfortably allows, the fix is trimming what we watch (fewer leaderboard symbols, fewer strikes/expirations), not raising this number back up toward the confirmed danger zone.
...
function createTradierHub({ token, onFlow, flowBuffer, onFlowEvent, onBackgroundFlow }) {
  ...
  let openedAt = null; // TEMP DIAGNOSTIC: set when 'open' fires, used to compute exact uptime before the next 'close' - remove alongside the other close-diagnostic additions

  // clientId -> { symbol, expiration, contracts: Map<occSymbol, {strike, type}> }
  const clientState = new Map();
  const quoteCache = new Map(); // occSymbol -> { bid, ask }

  // Persistent (NOT per-message) dedup for flow-history recording - the
  // per-message recordedKeys Set further down only protects against
  // several clients watching the same contract seeing the SAME incoming
  // trade message; it does nothing to stop the SAME real-world trade
  // being redelivered by Tradier after a reconnect and recorded a SECOND
  // time. Confirmed as a real problem in production: SPY alone
  // accumulated ~102,000 saved rows in a single day, far beyond
  // realistic volume, on a day with an unusual number of reconnects. Key
  // is a composite of everything that should be identical for the same
  // real trade (symbol/strike/type/price/size/timestamp) - two GENUINELY
  // different trades matching on all of these simultaneously is
  // vanishingly unlikely. Cleared periodically (not left to grow
  // forever) rather than tracked with per-entry expiry, since exact
  // redelivery after a reconnect happens close in time to the original,
  // not hours apart - a full periodic clear is simpler and sufficient.
  const recentlyRecordedTrades = new Set();
  const RECORDED_TRADES_CLEAR_INTERVAL_MS = 10 * 60_000;
  setInterval(() => recentlyRecordedTrades.clear(), RECORDED_TRADES_CLEAR_INTERVAL_MS);
            ...
            if (flowBuffer) {
              ...
              if (watcherState) {
                ...
                if (!recordedKeys.has(bufferKey)) {
                  ...
                  if (onFlowEvent && !isBigFlow && premium >= MIN_FLOW_PREMIUM_FOR_HISTORY) {
                    // Composite key identifying this specific real trade -
                    // if Tradier redelivers it after a reconnect, this
                    // catches it even though it arrives as a brand new
                    // message (a fresh recordedKeys Set, unaware anything
                    // was already recorded for this exact trade earlier).
                    const tradeKey = `${msg.symbol}:${price}:${size}:${timeMs}`;
                    if (!recentlyRecordedTrades.has(tradeKey)) {
                      recentlyRecordedTrades.add(tradeKey);
                      onFlowEvent({
    ...
    try {
      ...
      if (!socket || socket.readyState === WebSocket.CLOSED) {
        ...
        socket.on('close', (code, reasonBuf) => {
          // TEMP DIAGNOSTIC: the close event's code/reason were being
          // silently discarded entirely - added specifically because the
          // connection has been observed repeatedly failing to stay open
          // with NOTHING ever logged to explain why (no ws error, no
          // rebuild failure - just close, retry, close again). A
          // WebSocket close code (e.g. 1008 = policy violation, 1013 =
          // try again later/rate limited) directly reveals whether
          // Tradier itself is the one ending these connections, and why.
          ...
          console.warn(`[tradier] socket CLOSED: code=${code} reason="${reason}" uptimeMs=${uptimeMs}`);
