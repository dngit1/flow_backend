...
function createTradierHub({ token, onFlow, flowBuffer, onFlowEvent, onBackgroundFlow }) {
  ...
  // Periodically checks for two failure modes: a connection that LOOKS
  // open but has gone quiet (see lastMessageAt above - this is the
  // failure mode observed in production: no error, no close event, the
  // socket just stopped delivering trades until a manual restart), and -
  // just as important - NO connection at all while contracts are still
  // watched. The 'close' handler above already schedules one reconnect
  // attempt on its own, but if THAT attempt ever fails for any reason
  // (the sessionPromise bug fixed above was exactly this kind of gap),
  // nothing was left to notice and retry - this loop is the ongoing
  // safety net for that, checking every cycle rather than relying on a
  // single one-shot retry succeeding.
  setInterval(() => {
