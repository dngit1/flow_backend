...
function createTradierHub({ token, onFlow, flowBuffer, onFlowEvent, onBackgroundFlow }) {
  ...
  // quiet - see lastMessageAt above. Only fires while contracts are
  // actually being watched (a quiet connection with nothing subscribed
  // is normal, not stale) - this is exactly the failure mode observed in
  // production: no error, no close event, the socket just stopped
  // delivering trades until a manual restart.
  setInterval(() => {
