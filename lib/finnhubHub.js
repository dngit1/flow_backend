...
function createFinnhubHub({ apiKey, onTrade, onStatus }) {
  ...
  function ensureConnected() {
    ...
    socket.on('message', (raw) => {
      ...
      // TEMP DIAGNOSTIC: reveals the actual delivery pattern - real
      // per-trade pushes should show ~1 trade/message arriving steadily;
      // batching/throttling would show few messages with many trades
      // bundled into each one, or long gaps between messages. Remove
      // once the "candles only update once per bar" investigation is
      // resolved.
      const symbolsInMsg = [...new Set(msg.data.map((t) => t.s))].join(',');
      console.log(`[finnhub][msg] trades=${msg.data.length} symbols=${symbolsInMsg} receivedAt=${new Date().toISOString()}`);
